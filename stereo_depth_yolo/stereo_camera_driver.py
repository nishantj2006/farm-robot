...
class StereoCameraDriver(Node):
    def __init__(self):
        ...
        # Hardware-Accelerated GStreamer Pipeline (NVDEC Hardware JPEG Decoder)
        gst_pipelines = [
            # 1. Jetson NVDEC Hardware Accelerated Pipeline
            ...
            # 2. NVJPEG Hardware Decoder Fallback
            ("v4l2src device=/dev/video0 ! "
             "image/jpeg, width=2560, height=720, framerate=30/1 ! "
             "nvjpegdec ! videoconvert ! video/x-raw, format=BGR ! "
             "appsink drop=true max-buffers=1 sync=false"),
        ...
        self.cap = None
        for idx, pipeline in enumerate(gst_pipelines):
            self.get_logger().info(f"Attempting to open GStreamer pipeline option #{idx+1}...")
            cap = cv2.VideoCapture(pipeline, cv2.CAP_GSTREAMER)
            if cap.isOpened():
                self.cap = cap
                self.get_logger().info(f"Successfully initialized GStreamer pipeline option #{idx+1}")
                break

        # Fallback to standard V4L2 if all GStreamer pipelines fail
        if self.cap is None or not self.cap.isOpened():
            self.get_logger().warn("GStreamer pipelines failed. Falling back to native V4L2 driver...")
        ...
        # Pre-allocate padded image buffers once in memory
