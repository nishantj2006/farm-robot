...
class SpatialDetectionNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("3D Spatial Detection Node Initialized. Streaming to /spatial/annotated_image and /spatial/detections_3d...")
    ...
    def image_cb(self, msg):
        ...
        if self.latest_disparity_msg is not None and self.latest_detections:
            try:
                ...
                # Extract optical center parameters (cx_cam, cy_cam)
                if self.camera_info is not None:
                    cx_cam = self.camera_info.k[2]
                    cy_cam = self.camera_info.k[5]
                else:
                    # Fallback to image center if camera_info is not yet available
                ...
                for idx, det in enumerate(self.latest_detections):
                    ...
                    # Sample 25% central ROI to eliminate edge noise
                    ...
                    self.get_logger().info(
                        f"Detected '{label}' ({score:.2f}) at Spatial Posidtion: "
                    ...
                    det3d.bbox.size.z = 0.1  # Estimated depth bounds for object
                    detections_3d_msg.detections.append(det3d)

                    # --- Construct RViz Visual Marker ---
                    marker = Marker()
